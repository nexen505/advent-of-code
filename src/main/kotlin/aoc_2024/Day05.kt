        ...
        for (line in this@updates) {
            if (!line.contains('|')) {
                ...
                for (i in 0..<pages.size - 1) {
                    val pi = pages[i]
                    val ri = rules[pi] ?: emptySet()

                    if (!ri.containsAll(pages.subList(i + 1, pages.size))) {
                        correct = false
                        break
                    }
                }

                yield(pages to correct)
            }
        }
    }

private fun List<String>.extractRules(): Map<Long, Set<Long>> = asSequence()
    .map { it.split('|') }
    .filter { it.size == 2 }
    .map { (left, right) -> left.toLong() to right.toLong() }
    .groupingBy { it.first }
    .fold(emptySet()) { acc, el -> acc + el.second }
